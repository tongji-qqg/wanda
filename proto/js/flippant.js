...
function flip(flipper, content, type, class_name, timeout) {
  ...
  window.setTimeout(function () {
    ...
    var transform = $(flipper).css('transform');
    $(flipper).css({
  ...
  function close() {
    set_styles(back, flipper, position)
    back.classList.remove('flipper')
    back.classList.remove('flipped')
    flipper.classList.remove('flipped')
    var transform = $(flipper).attr('transform-temp');
    $(flipper).css({
    	'transform': transform
    });
    window.setTimeout(function () {
...
function set_styles(back, front, position) {
	...
	$back.css({
		'top':$front.offset().top,
		'left':$front.offset().left,
		'width':$front.width(),
		'min-height':$front.height(),
		'z-index':9999
	});
}
